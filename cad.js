...
export class Proj {
    constructor(inn) {
        ...
        try {
            ...
            // sess.err = error.stack
            alert(error.stack)
            sess.err = sess.err + error.stack
        }
    }
    ...
    markwrong(ko) {
        ...
        for (let e of bs[1]) {
            if (!/[lrgtfbcve]/.test(e)) {
                ...
                sess.erro = sess.erro + mark
            }
        }
        if (!/[0-9]/.test(bs[2])) {
            mark = bs[2].replace(bs[2], "<mark>" + bs[2] + "</mark>")
            sess.erro = sess.erro + mark
    ...
    makeKoxyz(ko) {
        ...
        i = 0
        for (let e of ko.lbs.slice(2)) {
            let re = new RegExp("[" + PMETER1 + "][0-9.-@]")
            if (re.test(e)) {
             f = this.splita1(e)
             if(f[1][0]=="@"){

                 ko[f[0]] += Number(f[1].slice(1))
             }else{
    ...
    getxyzfromcorner(left, di) {
        ...
        let partcorn = this.splita1(start[1])
            ...
            for (var j = 0; j < ko.nx[0]; j++) {
                for (var jj = 0; jj < ko.ny[0]; jj++) {
                    for (let jjj = 0; jjj < ko.nz[0]; jjj++) {
                        ...
                        let kk = { ...ko }
                        kk.x = ko.x + j * ko.nx[1]
                        kk.y = ko.y + jj * ko.ny[1]
                        kk.z = ko.z + jjj * ko.nz[1]
                        kk["nx"] = [1, 0]
                        kk["ny"] = [1, 0]
                        kk["nz"] = [1, 0]
                        kk["nme"] = nn
    ...
    makeValue2(s, prop, part = "l") {
        try {
            ...
            sess.err = sess.err+"false value: " + s + " in line: " + this.line
        }
    }
